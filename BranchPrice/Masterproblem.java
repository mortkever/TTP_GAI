...
public class Masterproblem {
    ...
    private List<BranchingDecision> arcFixations = new ArrayList<>();
    ...
    public int addTour(int team, Tour tour) {
        ...
        if(!isTourAllowed(team,tour)){
            return 0;
        }
        return tourRepo.addTour(team, tour);
    }

    private boolean isTourAllowed(int team, Tour tour) {
        for (BranchingDecision b : arcFixations) {
            if (b.team != team) continue;

            boolean arcInTour = tour.containsArc(b.from, b.to, b.slot);
            if (b.mustUseArc && !arcInTour) return false;
            if (!b.mustUseArc && arcInTour) return false;
        }
        return true;
    }
    ...
    public void setArcFixations(List<BranchingDecision> fixations) {
        this.arcFixations = fixations;
    }
    public Map<Integer, HashMap<Tour, GRBVar>> getLambdaVars() {
        return lambdaVars;
    }

    public void filterToursWithFixations(List<BranchingDecision> fixations) throws GRBException {
        Map<Integer, HashMap<Tour, GRBVar>> lambdaVars = this.getLambdaVars();

        // Bouw een set met verboden arcs
        Set<Arc> forbiddenArcs = new HashSet<>();
        for (BranchingDecision bd : fixations) {
            if (bd.getValue() == 0) {
                forbiddenArcs.add(bd.getArc());
            }
        }

        // Itereer over alle bestaande tours per team
        for (Map.Entry<Integer, HashMap<Tour, GRBVar>> entry : lambdaVars.entrySet()) {
            int team = entry.getKey();
            HashMap<Tour, GRBVar> tourVars = entry.getValue();

            List<Tour> toRemove = new ArrayList<>();
            for (Map.Entry<Tour, GRBVar> e : tourVars.entrySet()) {
                Tour tour = e.getKey();

                // Als een tour een verboden arc bevat, markeer voor verwijdering
                for (Arc arc : tour.getArcs()) {
                    if (forbiddenArcs.contains(arc)) {
                        toRemove.add(tour);
                        break;
                    }
                }
            ...
            // Verwijder alle foutieve tours
            for (Tour t : toRemove) {
                GRBVar var = tourVars.get(t);
                this.model.remove(var); // verwijder variabele uit model
                tourVars.remove(t);     // verwijder uit lambdaVars
            }
        }

        this.model.update(); // Gurobi-model updaten na verwijderingen
    }
}
